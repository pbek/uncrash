...
void Utils::Cli::generateFishCompletionScript(
    const QList<QCommandLineOption> &options, const QString &appName) {
  std::cout << "# Fish shell completion script for " << appName.toStdString()
            << std::endl;
  ...
  for (const QCommandLineOption &option : options) {
    for (const QString &name : option.names()) {
      ...
      //            QString cleanedName = name.startsWith("--") ? name.mid(2) :
      //            name.mid(1);
...
void Utils::Cli::generateBashCompletionScript(
    const QList<QCommandLineOption> &options, const QString &appName) {
  std::cout << "# Bash completion script for " << appName.toStdString()
            << std::endl;
  ...
  std::cout << "\"" << std::endl;
  std::cout << "  COMPREPLY=( $(compgen -W \"${commands}\" -- ${current}) )"
            << std::endl;
  std::cout << "  return 0" << std::endl;
  std::cout << "}" << std::endl;
  std::cout << "complete -F _" << appName.toStdString() << " "
            << appName.toStdString() << std::endl;
}

// Generates Zsh shell completion script
// TODO: Doesn't work yet
void Utils::Cli::generateZshCompletionScript(
    const QList<QCommandLineOption> &options, const QString &appName) {
  ...
  for (int i = 0; i < options.size(); ++i) {
    ...
    for (const QString &name : option.names()) {
      ...
      if (!option.valueName().isEmpty()) {
        // Assume the value can be any string, for specifics you might need to
        // adjust
        optionPattern +=
            QStringLiteral("=") + "'[:" + option.description() + ":]'";
      } else {
